...
The **Tick Ribbon** on the landing page is 800 consecutive real ETH/USDC swaps from Unichain
mainnet (`public/ribbon.bin`, generated by `backtest/export_ribbon.py`). Displacement is the
pool's tick. Only the segment inside the settlement window lights up, in the verdict's colour.

It is not an illustration of the data. It **is** the data. Do not replace it with a
decorative shape, and do not reuse it on other pages — spending it twice is what would make
it ordinary.
